...
const animationStores = new WeakMap<HTMLDivElement, IAnimationStore>();
...
function getAnimationStore(node: HTMLDivElement) {
  let store = animationStores.get(node);

  if (store == null) {
    store = {};

    animationStores.set(node, store);
  }
    ...
    onComplete() {
      ...
      store.timeout = setTimeout(() => {
        ...
        store.animation = fadeOut(controlsTextNode, {
          duration: animationTimings.fadeOut,
          onComplete() {
            store.animation = undefined;
          },
        });
      }, animationTimings.still);
    },
  });
}
