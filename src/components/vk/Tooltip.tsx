import { cloneElement, h, isValidElement } from "@external/preact";
import {
  useCallback,
  useEffect,
  useMemo,
...
import { uid } from "@external/uid";
import { getWindow } from "@utils/window";
import { wrapFunction } from "@utils/wrappers";

type VNode<P> = import("preact").VNode<P>;
...
const idRandomnessLength = 5;

/**
 * @return Уникальный ID для подсказки
 */
function createTooltipId() {
  return `tooltip_${uid(idRandomnessLength)}`;
}
...
export function Tooltip<P>(props: ITooltipProps<P>) {
  ...
  if (!isValidElement(children)) {
    ...
    throw new Error(notValidElementError);
  }
